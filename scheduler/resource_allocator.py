...
class ResourceAllocator:
    ...
    def add_node(self, node_id: str, cpu: float, memory: float):
        self.nodes[node_id] = Node(node_id=node_id, total_cpu=cpu, total_memory=memory)

    def allocate(self, task_id: str, cpu: float, memory: float, strategy: str = "best_fit") -> AllocationResult:
        candidates = [n for n in self.nodes.values() if n.available_cpu >= cpu and n.available_memory >= memory]
        if not candidates:
            return AllocationResult(success=False, message=f"No node has enough resources for {task_id}")
        if strategy == "first_fit":
            chosen = candidates[0]
        elif strategy == "best_fit":
            chosen = min(candidates, key=lambda n: n.available_cpu - cpu + n.available_memory - memory)
        else:
            chosen = max(candidates, key=lambda n: n.available_cpu + n.available_memory)
        chosen.used_cpu    += cpu
        chosen.used_memory += memory
        self.allocations[task_id] = chosen.node_id
        return AllocationResult(success=True, node_id=chosen.node_id, message=f"Allocated to {chosen.node_id}")
    ...
    def deallocate(self, task_id: str, cpu: float, memory: float):
        ...
        self.nodes[node_id].used_cpu    -= cpu
        self.nodes[node_id].used_memory -= memory
